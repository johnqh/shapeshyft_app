...
describe("BreadcrumbBuilder", () => {
  ...
  describe("localizedBreadcrumb", () => {
    it("returns translated title for known paths", () => {
      ...
      expect(builder.localizedBreadcrumb("/dashboard", mockT)).toBe(
        "Dashboard",
      );
    ...
    it("handles paths with language prefix", () => {
      expect(builder.localizedBreadcrumb("/en/dashboard", mockT)).toBe(
        "Dashboard",
      );
      expect(builder.localizedBreadcrumb("/de/pricing", mockT)).toBe("Pricing");
      expect(builder.localizedBreadcrumb("/ja/docs", mockT)).toBe(
        "Documentation",
      );
    });

    it("handles dashboard sub-paths", () => {
      expect(builder.localizedBreadcrumb("/dashboard/providers", mockT)).toBe(
        "Providers",
      );
      expect(builder.localizedBreadcrumb("/dashboard/analytics", mockT)).toBe(
        "Analytics",
      );
      expect(builder.localizedBreadcrumb("/dashboard/budgets", mockT)).toBe(
        "Budgets",
      );
      expect(builder.localizedBreadcrumb("/en/dashboard/settings", mockT)).toBe(
        "Settings",
      );
    });
    ...
    it("falls back to capitalized segment for unknown paths", () => {
      expect(builder.localizedBreadcrumb("/unknown-path", mockT)).toBe(
        "Unknown-path",
      );
  ...
  describe("getLocalizedBreadcrumbItems", () => {
    it("formats breadcrumbs for component use", () => {
      const result = builder.getLocalizedBreadcrumbItems(
        "/en/dashboard/providers",
        mockT,
      );
