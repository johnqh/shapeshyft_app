...
describe('auth utilities', () => {
  ...
  describe('formatFirebaseError', () => {
    ...
    it('returns fallback for errors without code', () => {
      expect(formatFirebaseError({})).toBe('Something went wrong. Please try again.');
      expect(formatFirebaseError(null)).toBe('Something went wrong. Please try again.');
    });

    it('returns fallback for unknown error codes', () => {
      const error = { code: 'auth/unknown' };
      expect(formatFirebaseError(error)).toBe('Something went wrong. Please try again.');
    });
  });
});
