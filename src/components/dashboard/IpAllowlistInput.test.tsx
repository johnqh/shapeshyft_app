  ...
  useTranslation: () => ({
    t: (key: string, options?: Record<string, unknown>) => {
      const translations: Record<string, string> = {
        ...
        'ipAllowlist.add': 'Add',
        'ipAllowlist.description': 'Restrict endpoint access to specific IP addresses.',
...
describe('IpAllowlistInput', () => {
  ...
  it('should render with empty state', () => {
    ...
    expect(screen.getByPlaceholderText('Enter IPv4 address (e.g., 192.168.1.1)')).toBeInTheDocument();
  });
  ...
  it('should add valid IP address', async () => {
    ...
    const input = screen.getByPlaceholderText('Enter IPv4 address (e.g., 192.168.1.1)');
  ...
  it('should show error for duplicate IP', async () => {
    ...
    expect(screen.getByText('This IP is already in the list')).toBeInTheDocument();
    expect(mockOnChange).not.toHaveBeenCalled();
  });
  ...
  it('should validate various valid IP formats', async () => {
    ...
    for (const ip of validIPs) {
      mockOnChange.mockClear();
      const { unmount } = render(<IpAllowlistInput value={[]} onChange={mockOnChange} />);
  ...
  it('should reject invalid IP formats', async () => {
    const user = userEvent.setup();
    const invalidIPs = ['256.1.1.1', '1.2.3', '1.2.3.4.5', 'abc.def.ghi.jkl', '192.168.1.1.'];
