...
class PerformanceAnalyzer {
  ...
  findFiles(pattern) {
    try {
      ...
      return files.filter(file => file.match(pattern)).map(file => path.join(assetsDir, file));
    } catch {
      return [];
    }
  }
  ...
  analyzeLoadTimes() {
    ...
    Object.entries(CONNECTION_SPEEDS).forEach(([connection, speed]) => {
      ...
      const timeGzip = this.calculateLoadTime(this.results.criticalPath.gzip, speed);
  ...
  analyzeBundles() {
    ...
    const totalJS = jsFiles.reduce((sum, file) => sum + this.getFileSize(file), 0);

    // Get all CSS files
    const cssFiles = this.findFiles(/\.css$/);
    const totalCSS = cssFiles.reduce((sum, file) => sum + this.getFileSize(file), 0);
    ...
    console.log(`Total CSS: ${colors.bright}${this.formatBytes(totalCSS)}${colors.reset}`);
    ...
    if (largestJS.length > 0) {
      ...
      largestJS.forEach((item, index) => {
        ...
        const sizeColor = item.size > 500000 ? colors.red : item.size > 200000 ? colors.yellow : colors.green;
        console.log(`${index + 1}. ${name}: ${sizeColor}${this.formatBytes(item.size)}${colors.reset}`);
      });
    }
    console.log('');
  }
  ...
  generateRecommendations() {
    ...
    if (criticalPath.gzip > BUDGETS.criticalPath) {
      recommendations.push({
        ...
        suggestion: 'Consider code splitting and lazy loading non-critical components',
      });
    } else {
      recommendations.push({
        type: 'success',
    ...
    // Total JS budget
    if (totalBundles.js > BUDGETS.totalJS * 10) { // More lenient for total since we have many lazy chunks
      recommendations.push({
        type: 'warning',
    ...
    if (criticalPath.uncompressed > 0) {
      const gzipRatio =
        ((criticalPath.uncompressed - criticalPath.gzip) / criticalPath.uncompressed) * 100;
  ...
  exportResults() {
    ...
    Object.entries(CONNECTION_SPEEDS).forEach(([connection, speed]) => {
      report.loadTimes[connection] = {
        ...
        uncompressed: this.calculateLoadTime(this.results.criticalPath.uncompressed, speed),
      };
    });

    fs.writeFileSync(outputPath, JSON.stringify(report, null, 2));
    console.log(`📄 Detailed report exported to: ${colors.cyan}${outputPath}${colors.reset}\n`);
  }

  // Main analysis function
  async analyze() {
    console.log(`${colors.bright}${colors.cyan}🚀 ShapeShyft Performance Monitor${colors.reset}\n`);
    ...
    console.log(`${colors.green}🏁 Performance analysis complete!${colors.reset}`);
  }
}
